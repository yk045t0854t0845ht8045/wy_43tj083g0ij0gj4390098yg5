...
type AzureStatePayload = {
  typ: "wz-azure-oauth-state";
  next: string;
...
function normalizeMicrosoftPhoneToE164Br(value?: string | null) {
...
function extractMicrosoftPhoneCandidate(user: Record<string, unknown>) {
  ...
  for (const candidate of candidates) {
    const normalized = normalizeMicrosoftPhoneToE164Br(candidate);
...
function clearAzureStateCookie(res: NextResponse, req: NextRequest) {
  ...
  const domain = resolveMicrosoftStateCookieDomain(req);
  clearByName(AZURE_STATE_COOKIE_NAME);
  clearByName(AZURE_CONNECT_STATE_COOKIE_NAME);
  if (domain) {
    clearByName(AZURE_STATE_COOKIE_NAME, domain);
    clearByName(AZURE_CONNECT_STATE_COOKIE_NAME, domain);
  }
}

function readAzureStateTicket(ticket: string) {
  const secret = getTicketSecret();
  if (!secret) {
  ...
  try {
    const parsed = JSON.parse(base64UrlDecodeToString(payloadB64)) as AzureStatePayload;
    if (parsed?.typ !== "wz-azure-oauth-state") {
      return { ok: false as const, error: "State OAuth invalido." };
    }
...
function buildAzureOnboardingRedirect(params: {
  origin: string;
  next: string;
  email: string;
}) {
  ...
  url.searchParams.set("oauthProvider", "azure");
  url.searchParams.set("oauthEmail", params.email);
  url.searchParams.set("oauthStep", "email");
  return url.toString();
}

function readAzureIntent(payload?: AzureStatePayload | null) {
  return payload?.intent === "connect" ? "connect" : "login";
}

function buildAzureConnectRedirect(params: {
  origin: string;
  next: string;
  ok: boolean;
  error?: string;
...
async function findWzUserById(params: {
  ...
  for (const columns of columnsToTry) {
    ...
    if (!hasMissingColumn) {
      console.error("[azure-callback] find wz_user by id error:", res.error);
      break;
    }
  }
...
async function findOrCreateMicrosoftWzUser(params: {
  sb: ReturnType<typeof supabaseAdmin>;
  email: string;
  fullName: string | null;
  authUserId: string;
...
function parseAzureProviderUserId(user: Record<string, unknown>) {
  const identities = Array.isArray(user?.identities)
    ? (user.identities as Array<Record<string, unknown>>)
    : [];
  const azureIdentity = identities.find(
    (identity) => String(identity?.provider || "").trim().toLowerCase() === "azure",
  );
  if (!azureIdentity) return null;

  const directId = normalizeText(String(azureIdentity.id || ""));
  if (directId) return directId;

  const identityData =
    azureIdentity.identity_data && typeof azureIdentity.identity_data === "object"
      ? (azureIdentity.identity_data as Record<string, unknown>)
      : null;
...
async function upsertPendingAzureOnboarding(params: {
  sb: ReturnType<typeof supabaseAdmin>;
  email: string;
  authUserId: string;
  fullName: string | null;
...
async function tryTrustedAzureBypass(params: {
  sb: ReturnType<typeof supabaseAdmin>;
  req: NextRequest;
  email: string;
  userId: string;
  ...
  if (trustedErr) {
    console.error("[azure-callback] trusted device lookup error:", trustedErr);
    return null;
  }
...
function appendAzurePasswordSetupQuery(nextSafe: string) {
  ...
  target.searchParams.set("passwordSetupProvider", "azure");
...
async function exchangeAzurePkceCode(params: {
  code: string;
  codeVerifier: string;
}) {
...
export async function GET(req: NextRequest) {
  ...
  const stFromCookie = String(req.cookies.get(AZURE_STATE_COOKIE_NAME)?.value || "").trim();
  const connectStateFromCookie = String(
    req.cookies.get(AZURE_CONNECT_STATE_COOKIE_NAME)?.value || "",
  ).trim();
  const hasExplicitStQuery = Boolean(stFromQuery);

  const st = stFromQuery || stFromCookie;
  const primaryStateRes = readAzureStateTicket(st);
  const connectCookieStateRes = readAzureStateTicket(connectStateFromCookie);
  let stateRes = primaryStateRes;
  if (connectCookieStateRes.ok) {
    if (!stateRes.ok) {
      stateRes = connectCookieStateRes;
    } else if (
      readAzureIntent(stateRes.payload) !== "connect" &&
      !hasExplicitStQuery
    ) {
  ...
  const oauthIntent = stateRes.ok ? readAzureIntent(stateRes.payload) : "login";

  const fail = (message: string) => {
    const target =
      oauthIntent === "connect"
        ? buildAzureConnectRedirect({
  ...
  try {
    ...
    if (!identityLookup.lookupOk) {
      if (!identityLookup.schemaReady) {
        return fail("Schema de provedores nao disponivel para validar vinculo Microsoft.");
      }
      return fail(
    ...
    if (oauthIntent === "connect") {
      ...
      const successUrl = buildAzureConnectRedirect({
        origin: dashboardOrigin,
        next: safeNext,
        ok: true,
      });
    ...
    if (!loginUpsert.ok) {
      if (!loginUpsert.schemaReady) {
        return fail("Schema de provedores nao disponivel para concluir login Microsoft.");
      }
      if (loginUpsert.reason === "identity-conflict") {
        return fail("Esta conta Microsoft ja esta conectada a outra conta.");
      }
      console.error("[azure-callback] failed to persist Microsoft login provider link");
      return fail("Nao foi possivel concluir o login Microsoft agora. Tente novamente.");
    }

    const azurePhoneCandidate = extractMicrosoftPhoneCandidate(user);
    ...
    if (!currentPhone && azurePhoneCandidate) {
      await updateWzUserBestEffort({
        ...
        userId: wzUser.userId,
        patch: {
          phone_e164: azurePhoneCandidate,
        },
      });
    }
    ...
    if (!wzUser.isNew) {
      ...
      if (trustedRedirect) {
        clearAzureStateCookie(trustedRedirect, req);
    ...
    const onboardingNext = wzUser.mustCreatePassword
      ...
      : safeNext;
    const successUrl = buildAzureOnboardingRedirect({
      origin: requestOrigin,
      next: onboardingNext,
      email: accountEmail,
    });
