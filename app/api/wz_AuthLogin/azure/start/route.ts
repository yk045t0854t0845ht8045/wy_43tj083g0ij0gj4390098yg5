...
function resolveAzureStateCookieDomain(req: NextRequest) {
  const host = String(
    req.headers.get("x-forwarded-host") || req.headers.get("host") || req.nextUrl.host || "",
  )
    .split(":")[0]
...
function createAzureStateTicket(params: {
  next: string;
  intent?: "login" | "connect";
  ...
  const payload: AzureStatePayload = {
    typ: "wz-azure-oauth-state",
...
export async function POST(req: NextRequest) {
  try {
    ...
    const oauthScopes =
      String(process.env.AZURE_OAUTH_SCOPES || "openid profile email").trim() ||
      "openid profile email";

    const stateTicket = createAzureStateTicket({
      next: nextSafe,
      intent,
      connectUserId,
      connectSid,
    ...
    if (error || !data?.url) {
      console.error("[azure-start] signInWithOAuth error:", error);
      return NextResponse.json(
        {
          ok: false,
          error:
    ...
    const cookieDomain = resolveAzureStateCookieDomain(req);
    response.cookies.set({
      name: AZURE_STATE_COOKIE_NAME,
      value: stateTicket,
      httpOnly: true,
    ...
    if (intent === "connect") {
      response.cookies.set({
        name: AZURE_CONNECT_STATE_COOKIE_NAME,
        value: stateTicket,
        httpOnly: true,
        secure: process.env.NODE_ENV === "production",
        sameSite: "lax",
