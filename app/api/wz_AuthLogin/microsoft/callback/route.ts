...
function clearMicrosoftStateCookie(res: NextResponse, req: NextRequest) {
  res.cookies.set({
...
function readMicrosoftStateTicket(ticket: string) {
  const secret = getTicketSecret();
  if (!secret) {
  ...
  try {
    const parsed = JSON.parse(base64UrlDecodeToString(payloadB64)) as MicrosoftStatePayload;
    if (parsed?.typ !== "wz-microsoft-oauth-state") {
      return { ok: false as const, error: "State OAuth invalido." };
    }
...
function buildMicrosoftOnboardingRedirect(params: {
  origin: string;
  next: string;
  email: string;
}) {
...
function readMicrosoftIntent(payload?: MicrosoftStatePayload | null) {
  return payload?.intent === "connect" ? "connect" : "login";
}

function buildMicrosoftConnectRedirect(params: {
  origin: string;
  next: string;
  ok: boolean;
  error?: string;
...
async function findWzUserById(params: {
  ...
  for (const columns of columnsToTry) {
    ...
    if (!hasMissingColumn) {
      console.error("[microsoft-callback] find wz_user by id error:", res.error);
      break;
    }
  }
...
function parseMicrosoftProviderUserId(user: Record<string, unknown>) {
  const identities = Array.isArray(user?.identities)
    ? (user.identities as Array<Record<string, unknown>>)
    : [];
  ...
  const identityData =
    microsoftIdentity.identity_data && typeof microsoftIdentity.identity_data === "object"
      ? (microsoftIdentity.identity_data as Record<string, unknown>)
      : null;
...
async function upsertPendingMicrosoftOnboarding(params: {
  sb: ReturnType<typeof supabaseAdmin>;
  email: string;
  authUserId: string;
  fullName: string | null;
...
async function tryTrustedMicrosoftBypass(params: {
  sb: ReturnType<typeof supabaseAdmin>;
  req: NextRequest;
  email: string;
  userId: string;
  ...
  if (trustedErr) {
    console.error("[microsoft-callback] trusted device lookup error:", trustedErr);
    return null;
  }
...
async function exchangeMicrosoftPkceCode(params: {
  code: string;
  codeVerifier: string;
}) {
...
export async function GET(req: NextRequest) {
  ...
  const stFromCookie = String(req.cookies.get(MICROSOFT_STATE_COOKIE_NAME)?.value || "").trim();
  const st = stFromQuery || stFromCookie;
  const stateRes = readMicrosoftStateTicket(st);
  const safeNext = stateRes.ok ? sanitizeNext(stateRes.payload.next) : "/";
  const oauthIntent = stateRes.ok ? readMicrosoftIntent(stateRes.payload) : "login";

  const fail = (message: string) => {
    const target =
      oauthIntent === "connect"
        ? buildMicrosoftConnectRedirect({
    ...
    clearMicrosoftStateCookie(res, req);
    applyNoStore(res);
    return res;
  };
  ...
  try {
    ...
    const nowIso = new Date().toISOString();
    const providerUserId = parseMicrosoftProviderUserId(user);
    ...
    const microsoftPhoneCandidate = extractMicrosoftPhoneCandidate(user);
    const currentPhone = normalizeOptionalPhone(wzUser.phoneE164);
    const effectivePhone = currentPhone || microsoftPhoneCandidate || null;

    if (!currentPhone && microsoftPhoneCandidate) {
      await updateWzUserBestEffort({
        sb,
        userId: wzUser.userId,
        patch: {
          phone_e164: microsoftPhoneCandidate,
        },
      });
    }

    if (!wzUser.isNew) {
      const trustedRedirect = await tryTrustedMicrosoftBypass({
        sb,
        req,
        email: accountEmail,
        userId: wzUser.userId,
      ...
      if (trustedRedirect) {
        clearMicrosoftStateCookie(trustedRedirect, req);
        applyNoStore(trustedRedirect);
        return trustedRedirect;
      }
    }

    await upsertPendingMicrosoftOnboarding({
      sb,
      email: accountEmail,
      authUserId,
      fullName: wzUser.fullName || fullName,
    ...
    const successUrl = buildMicrosoftOnboardingRedirect({
      origin: requestOrigin,
      next: safeNext,
      email: accountEmail,
    });
