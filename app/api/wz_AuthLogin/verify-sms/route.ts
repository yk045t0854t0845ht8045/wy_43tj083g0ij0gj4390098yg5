...
export async function POST(req: Request) {
  try {
    ...
    if (String(pend.data.stage || "") !== "sms") {
      return NextResponse.json({ ok: false, error: "Conclua o e-mail primeiro." }, { status: 400 });
    }
    ...
    if (exErr) {
      ...
      return NextResponse.json({ ok: false, error: "Falha ao validar cadastro." }, { status: 500 });
    }
    ...
    if (existing?.id) {
      ...
      if (upUserErr) {
        ...
        return NextResponse.json({ ok: false, error: "Falha ao atualizar cadastro." }, { status: 500 });
      }
    } else {
      ...
      if (insErr || !createdRow?.id) {
        ...
        return NextResponse.json({ ok: false, error: "Falha ao salvar cadastro." }, { status: 500 });
      }
    ...
    // ✅ URL FINAL ABSOLUTA no dashboard (sem /dashboard no path)
