...
import { normalizeEmail, normalizeOptionalText } from "@/app/api/wz_users/_onboarding";
import {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const companyOnboardingId =
      normalizeOptionalText(String(req.nextUrl.searchParams.get("companyOnboardingId") || "")) ||
      normalizeOptionalText(String(formData.get("companyOnboardingId") || ""));
    ...
    const onboarding = await ensureCompanyOnboardingRecord({
      sb,
      sessionUserId,
      sessionEmail,
      companyOnboardingId,
      ...
      return NextResponse.json(
        {
          ...
          error:
            onboarding.schemaReady === false
              ? COMPANY_ONBOARDING_SCHEMA_HINT
              : "Nao foi possivel localizar onboarding da empresa para salvar a logo.",
        },
    ...
    const objectPath = `${onboarding.record.userId}/companies/${onboarding.record.id}/logo-${Date.now()}.${extension}`;
    ...
    if (uploadError) {
      console.error("[company-onboarding-logo] upload error:", uploadError);
      return NextResponse.json(
    ...
    const updated = await patchCompanyOnboardingRecord({
      sb,
    ...
    if (!updated.ok) {
      console.error("[company-onboarding-logo] update onboarding error:", updated.error);
      ...
      return NextResponse.json(
        {
          ...
          error:
            updated.schemaReady === false
              ? COMPANY_ONBOARDING_SCHEMA_HINT
              : "Nao foi possivel salvar a logo no onboarding da empresa.",
        },
    ...
    try {
      await cleanupCompanyOnboardingLogoFolder({
        ...
        companyOnboardingId: onboarding.record.id,
        keepObjectPath: objectPath,
      });
    } catch (cleanupError) {
      console.error("[company-onboarding-logo] cleanup stale logos error:", cleanupError);
    }

    const refreshed = await ensureCompanyOnboardingRecord({
      sb,
      sessionUserId,
      sessionEmail,
      companyOnboardingId: onboarding.record.id,
    ...
    console.error("[company-onboarding-logo] unexpected error:", error);
    return NextResponse.json(
      { ok: false, error: "Erro inesperado ao enviar logo da empresa." },
      { status: 500, headers: NO_STORE_HEADERS },
    );
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const companyOnboardingId = normalizeOptionalText(
      String(req.nextUrl.searchParams.get("companyOnboardingId") || ""),
    );
    if (!companyOnboardingId) {
      return NextResponse.json(
        { ok: false, error: "companyOnboardingId e obrigatorio para remover a logo." },
        { status: 400, headers: NO_STORE_HEADERS },
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          error:
            onboarding.schemaReady === false
              ? COMPANY_ONBOARDING_SCHEMA_HINT
              : "Nao foi possivel localizar onboarding da empresa para remover a logo.",
        },
        { status: onboarding.schemaReady === false ? 500 : 400, headers: NO_STORE_HEADERS },
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          error:
            updated.schemaReady === false
              ? COMPANY_ONBOARDING_SCHEMA_HINT
              : "Nao foi possivel remover a logo da empresa.",
        },
        { status: updated.schemaReady === false ? 500 : 400, headers: NO_STORE_HEADERS },
      );
    }
    ...
    try {
      ...
      console.error("[company-onboarding-logo] cleanup folder on delete error:", cleanupError);
    }
    ...
    console.error("[company-onboarding-logo] unexpected delete error:", error);
    return NextResponse.json(
      { ok: false, error: "Erro inesperado ao remover logo da empresa." },
      { status: 500, headers: NO_STORE_HEADERS },
    );
  }
}
