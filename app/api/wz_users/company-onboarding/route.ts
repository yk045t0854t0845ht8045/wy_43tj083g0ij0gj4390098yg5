...
import {
  ...
  isCompanyOnboardingSchemaError,
...
type ResolvedCompanyOnboardingContext = {
...
function hasCompleteTeamData(onboarding: CompanyOnboardingRecord) {
...
async function refreshOnboarding(params: ResolvedCompanyOnboardingContext) {
  const refreshed = await ensureCompanyOnboardingRecord({
    sb: params.sb,
    sessionUserId: params.sessionUserId,
    sessionEmail: params.sessionEmail,
    companyOnboardingId: params.onboarding.id,
...
async function patchOnboardingAndRefresh(params: {
  ctx: ResolvedCompanyOnboardingContext;
...
async function syncWhatsAppWithProvider(params: {
  ...
  const instanceName = buildOwnWhatsAppInstanceName(
    params.ctx.onboarding.userId,
    params.ctx.onboarding.id,
  );
  registerWhatsAppInstanceBinding({
    instanceName,
    userId: params.ctx.onboarding.userId,
    onboardingId: null,
    companyOnboardingId: params.ctx.onboarding.id,
...
async function withCompanyOnboardingContext(
  req: NextRequest,
  opts?: { companyOnboardingId?: string | null; createIfMissing?: boolean },
) {
  ...
  const onboarding = await ensureCompanyOnboardingRecord({
    ...
    sessionUserId,
    sessionEmail,
    companyOnboardingId: normalizeOptionalText(opts?.companyOnboardingId || ""),
  ...
  if (!onboarding.ok) {
    ...
    const errorMessage =
      onboarding.schemaReady === false
        ...
        : getErrorMessage(
            onboarding.error,
            "Nao foi possivel carregar onboarding adicional da empresa.",
          );
    if (onboarding.error && !isCompanyOnboardingSchemaError(onboarding.error)) {
      console.error("[company-onboarding] ensure record error:", onboarding.error);
    }
    return {
      ok: false as const,
      response: NextResponse.json(
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { ok: false, error: "Erro inesperado ao carregar onboarding adicional." },
      { status: 500, headers: NO_STORE_HEADERS },
    );
  }
}
  ...
  try {
    ...
    if (action === "save-company") {
      ...
      try {
        ...
        console.error("[company-onboarding] cleanup stale company logos error:", cleanupError);
      }
    ...
    return NextResponse.json(
      { ok: false, error: "Erro inesperado ao salvar onboarding adicional." },
      { status: 500, headers: NO_STORE_HEADERS },
    );
  }
}
